...
This returned byte data is sent over Pyserial to the mainboard at a baud rate of 115200     +
``
...
Here, no data is received from the mainboard, as we have implemented only a one-way communication between the mainboard and the system(NUC), over which the software (main program) sends motor speeds over the serial bus, and the mainboard drives the motors in that particular speed.
